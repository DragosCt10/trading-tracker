...
export default function SearchBar() {
  ...
  return (
    <div className="relative w-full max-w-sm">
      ...
      {/* Type tabs */}
      {showDropdown && (
        <div className="absolute top-full mt-1.5 w-full rounded-2xl border border-slate-200/70 dark:border-slate-700/60 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl shadow-xl shadow-slate-900/10 dark:shadow-2xl dark:shadow-black/40 overflow-hidden z-50">
          {/* Tabs */}
            ...
            {isFetching ? (
              ...
              (data as FeedPost[]).map((post) => (
                <Link
                  ...
                  className="flex flex-col gap-0.5 px-4 py-3 hover:bg-slate-100/90 dark:hover:bg-slate-800/50 transition-colors border-b border-slate-100 dark:border-slate-800/60 last:border-0"
                >
              ...
              (data as SocialProfile[]).map((profile) => (
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-3 hover:bg-slate-100/90 dark:hover:bg-slate-800/50 transition-colors border-b border-slate-100 dark:border-slate-800/60 last:border-0"
                >

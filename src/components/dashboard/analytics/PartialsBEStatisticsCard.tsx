...
/**
 * Compute Partials & BE stats from trades (Partials Taken + Break Even).
 * Wins/losses use trade_outcome or be_final_result for BE trades.
 */
function getPartialsBEStats(trades: Trade[]) {
  const partialsBETrades = trades.filter((t) => t.break_even && t.partials_taken);
  const wins = partialsBETrades.filter((t) => {
    if (t.trade_outcome === 'BE') return t.be_final_result === 'Win';
    return t.trade_outcome === 'Win';
  }).length;
  const losses = partialsBETrades.filter((t) => {
    if (t.trade_outcome === 'BE') return t.be_final_result === 'Lose';
    return t.trade_outcome === 'Lose';
  }).length;
  const totalTrades = partialsBETrades.length;
...
export function getPartialsBEChartData(trades: Trade[]) {
  const stats = getPartialsBEStats(trades);
  return [
    {
...
const LEGEND_LABEL = 'Partials + BE';
...
export const PartialsBEStatisticsCard: React.FC<PartialsBEStatisticsCardProps> = React.memo(
  function PartialsBEStatisticsCard({ trades, isLoading: externalLoading }) {
    ...
    const pieData = [
      { name: 'Wins', value: stats.wins, color: 'emerald', gradientId: 'partialsBEWins', pct: totalTrades > 0 ? (stats.wins / totalTrades) * 100 : 0 },
      { name: 'Losses', value: stats.losses, color: 'rose', gradientId: 'partialsBELosses', pct: totalTrades > 0 ? (stats.losses / totalTrades) * 100 : 0 },
    ].filter((d) => d.value > 0);
        ...
        <Card className="relative overflow-hidden border-slate-200/60 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/30 shadow-lg shadow-slate-200/50 dark:shadow-none backdrop-blur-sm h-96 flex flex-col">
          <CardHeader className="pb-2 flex-shrink-0">
            ...
            <CardDescription className="text-base text-slate-500 dark:text-slate-400">
              Analysis of trades marked as both Break Even and Partials Taken
            </CardDescription>
          </CardHeader>
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <TooltipProvider>
              <UITooltip delayDuration={150}>
                ...
                >
                  <div className="relative text-xs sm:text-sm text-slate-500 dark:text-slate-400">For accurate stats, select BE as trade outcome and also set After BE (Win or Lose).</div>
                </UITooltipContent>
              </UITooltip>
            </TooltipProvider>
          </div>
          ...
          <div className="flex-1 w-full flex items-center justify-center min-h-0 relative">
            <div className="w-full h-full max-h-[200px] relative">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <defs>
                    <linearGradient id="partialsBEWins" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#10b981" stopOpacity={1} />
                      <stop offset="50%" stopColor="#14b8a6" stopOpacity={0.95} />
                      <stop offset="100%" stopColor="#0d9488" stopOpacity={0.9} />
                    </linearGradient>
                    <linearGradient id="partialsBELosses" x1="0" y1="0" x2="0" y2="1">

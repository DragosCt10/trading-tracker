...
export const SLSizeStatisticsCard: React.FC<SLSizeStatisticsCardProps> = React.memo(
  function SLSizeStatisticsCard({ slSizeStats, isLoading: externalLoading }) {
    ...
    const chartBarsData = slSizeStats.map((stat) => ({
      name: stat.market,
      value: Number(stat.averageSlSize.toFixed(2)),
      market: stat.market,
      averageSlSize: stat.averageSlSize,
    }));
          ...
          <div className="relative flex flex-col gap-3">
            ...
            <div className="space-y-2">
              <div className="flex items-baseline justify-between gap-4">
                <span className="text-xs font-medium text-slate-600 dark:text-slate-400">Avg SL Size</span>
                <span className="text-lg font-bold text-slate-900 dark:text-slate-100">
                  {d.averageSlSize.toFixed(2)}
                </span>
              </div>
            </div>
          </div>
        ...
        <Card className="relative overflow-hidden border-slate-200/60 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/30 shadow-lg shadow-slate-200/50 dark:shadow-none backdrop-blur-sm h-[420px] flex flex-col">
          <CardHeader className="pb-2 flex-shrink-0">
            <CardTitle className="text-lg font-semibold bg-gradient-to-br from-slate-900 to-slate-700 dark:from-slate-100 dark:to-slate-300 bg-clip-text text-transparent mb-1">
              Stop Loss Size Stats
            </CardTitle>
            <CardDescription className="text-base text-slate-500 dark:text-slate-400">
              Distribution of trades based on SL size
            </CardDescription>
          </CardHeader>
    ...
    if (!slSizeStats.length || chartBarsData.length === 0) {
      return (
        <Card className="relative overflow-hidden border-slate-200/60 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/30 shadow-lg shadow-slate-200/50 dark:shadow-none backdrop-blur-sm h-[420px] flex flex-col">
          <CardHeader className="pb-2 flex-shrink-0">
            <CardTitle className="text-lg font-semibold bg-gradient-to-br from-slate-900 to-slate-700 dark:from-slate-100 dark:to-slate-300 bg-clip-text text-transparent mb-1">
          ...
          <CardContent className="flex-1 flex flex-col items-center justify-center">
            <div className="text-base font-medium text-slate-600 dark:text-slate-300 text-center mb-1">
              No SL size data found
            </div>
            <div className="text-sm text-slate-500 dark:text-slate-400 text-center max-w-xs">
              Add trades with SL Size to see average per market here.
            </div>
          </CardContent>
        </Card>
      );
          ...
          <div className="flex-1 w-full flex items-center justify-center min-h-0 relative pl-1 pr-4">
            <div className="w-full h-full relative">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <defs>
                    <linearGradient id="slSizeGradient" x1="0" y1="0" x2="1" y2="0">

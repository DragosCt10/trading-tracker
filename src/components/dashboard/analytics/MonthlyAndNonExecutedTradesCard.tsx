...
export interface MonthlyAndNonExecutedTradesCardProps {
...
export const MonthlyAndNonExecutedTradesCard: React.FC<MonthlyAndNonExecutedTradesCardProps> = React.memo(
  function MonthlyAndNonExecutedTradesCard({ 
    monthlyStats,
    ...
    // Calculate total executed trades from monthly stats
    const totalExecutedTrades = useMemo(() => {
      if (!monthlyStats.monthlyData) {
        return 0;
      }
      return Object.values(monthlyStats.monthlyData).reduce(
        (sum, month) =>
          sum +
          month.wins +
          month.losses +
        ...
        <Card className="relative overflow-hidden border-slate-300/40 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/30 shadow-lg shadow-slate-200/50 dark:shadow-none backdrop-blur-sm h-96 flex flex-col">
          <CardHeader className="pb-2 flex-shrink-0">
            <CardTitle className="text-lg font-semibold bg-gradient-to-br from-slate-900 to-slate-700 dark:from-slate-100 dark:to-slate-300 bg-clip-text text-transparent mb-1">
              Monthly & Non-Executed Trades
            </CardTitle>
            <CardDescription className="text-base text-slate-500 dark:text-slate-400">
              Average monthly trades and non-executed trades overview
            </CardDescription>
          </CardHeader>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    ...
                    content={(props) => <CustomTooltip {...props} totalTrades={totalTrades} />}
                  />
                </PieChart>
              </ResponsiveContainer>
          ...
          <div className="w-full px-4 pt-4 mt-2">
            <div className="flex items-center justify-center gap-8">
              <div className="flex flex-col items-center">
                ...
                <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">(incl. BE)</div>
              </div>

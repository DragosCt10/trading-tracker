...
export const AverageDrawdownChart = React.memo(function AverageDrawdownChart({ averageDrawdown }: AverageDrawdownChartProps) {
  ...
  // Normalize average drawdown to 0-100% for display (cap at 20% for visual purposes)
  const normalizedValue = Math.max(0, Math.min(averageDrawdown, 20));
  const percentage = (normalizedValue / 20) * 100;
  const remainingPercentage = 100 - percentage;

  const data = [
    { name: 'Average Drawdown', value: percentage },
    { name: 'Remaining', value: remainingPercentage },
  ];

  // Determine color based on average drawdown value (lower is better)
  const getGradientId = () => {
    if (averageDrawdown <= 2) return 'drawdownBlue';
    if (averageDrawdown <= 5) return 'drawdownEmerald';
    if (averageDrawdown <= 10) return 'drawdownAmber';
    if (averageDrawdown <= 15) return 'drawdownOrange';
    return 'drawdownRed';
  };
            ...
            <div className="relative overflow-hidden rounded-xl p-3 border border-slate-200/70 dark:border-slate-800/70 bg-slate-50/80 dark:bg-slate-900/70 backdrop-blur-xl shadow-lg shadow-slate-900/5 dark:shadow-black/40">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                  Average Drawdown: <span className={cn("font-bold", getTooltipValueColor())}>{averageDrawdown.toFixed(2)}%</span>
                </div>
              </div>

...
/**
 * Convert MSS stats to chart data format (kept for compatibility)
 */
export function convertMssStatsToChartData(
  ...
  return mssStats.map((stat) => {
    const totalTrades = stat.total ?? (stat.wins ?? 0) + (stat.losses ?? 0) + (stat.breakEven ?? 0);
    return {
      category: stat.mss ?? '',
      wins: stat.wins ?? 0,
 ...
 */
export function convertFilteredMssStatsToChartData(mssStats: MssStatsLike[]) {
...
export const MSSStatisticsCard: React.FC<MSSStatisticsCardProps> = React.memo(
  function MSSStatisticsCard({ mssStats, isLoading: externalLoading, includeTotalTrades = false }) {
    ...
    const totalTrades = mssStats.reduce(
      (sum, s) => sum + (s.total ?? (s.wins ?? 0) + (s.losses ?? 0) + (s.breakEven ?? 0)),
      0
    );

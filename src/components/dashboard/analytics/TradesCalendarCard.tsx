...
export const TradesCalendarCard: React.FC<TradesCalendarCardProps> = ({
  ...
  // Week layout starts on Sunday and ends on Saturday
  // JS getDay(): 0 = Sun, 1 = Mon, ..., 6 = Sat
  const firstDayOfWeek = firstDay.getDay();
  const weekdayOrder = [0, 1, 2, 3, 4, 5, 6]; // Sun, Mon, Tue, Wed, Thu, Fri, Sat
            ...
            {[...emptyCells, ...daysInMonth].map((date, index) => {
              ...
              const baseColor =
                displayProfit > 0
                  ...
                  ? 'bg-slate-500/10 dark:bg-slate-500/20 border border-slate-300 dark:border-slate-600 hover:bg-slate-500/15 dark:hover:bg-slate-500/25 hover:border-slate-400 dark:hover:border-slate-500'
                  : hasBE && beOutcome === 'Win'
                  ? 'bg-emerald-500/10 dark:bg-emerald-500/20 border border-emerald-200 dark:border-emerald-800 hover:bg-emerald-500/15 dark:hover:bg-emerald-500/28 hover:border-emerald-300 dark:hover:border-emerald-700'
                ...
                return (
                  <Tooltip key={date.toString()} delayDuration={160}>
                    ...
                    <TooltipContent
                      side="top"
                      align="center"
                      className="w-56 rounded-2xl p-4 relative overflow-hidden border border-slate-300/40 dark:border-slate-700/50 bg-slate-50/80 dark:bg-slate-800/90 backdrop-blur-xl shadow-lg shadow-slate-900/5 dark:shadow-black/40 text-slate-900 dark:text-slate-100"
                      sideOffset={6}
                    >

...
export const RecoveryFactorChart = React.memo(function RecoveryFactorChart({
  ...
  const data = [
    { name: 'Recovery Factor', value: percentage },
    { name: 'Remaining',       value: remainingPercentage },
  ];

  const getGradientId = () => {
    if (recoveryFactor < 0.5) return 'recoveryRed';
    if (recoveryFactor < 1.0) return 'recoveryAmber';
    if (recoveryFactor < 2.0) return 'recoveryYellow';
    if (recoveryFactor < 3.0) return 'recoveryEmerald';
    return 'recoveryBlue';
  };
  ...
  const displayValue = recoveryFactor >= MAX_SCALE
    ? '5+'
    ...
    return (
      <Card className="relative overflow-hidden border-slate-300/40 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/30 shadow-lg shadow-slate-200/50 dark:shadow-none backdrop-blur-sm">
        <CardHeader className="pb-2 flex-shrink-0">
          ...
          <CardDescription className="text-base text-slate-500 dark:text-slate-400 mb-3">Profit vs max drawdown</CardDescription>
        </CardHeader>
      ...
      <div className={cn('relative z-0', isLocked && 'blur-[3px] opacity-70 pointer-events-none select-none')}>
              ...
              <div className="relative flex flex-col">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                    Recovery Factor: <span className={cn('font-bold', getTextColor())}>{displayValue}</span>
                  </div>
                </div>
        ...
        <div className="w-full h-full">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <defs>
                <linearGradient id="recoveryRed" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#f43f5e" stopOpacity={1} />
                  <stop offset="100%" stopColor="#e11d48" stopOpacity={0.9} />
                </linearGradient>
                <linearGradient id="recoveryAmber" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#f59e0b" stopOpacity={1} />
                  <stop offset="100%" stopColor="#d97706" stopOpacity={0.9} />
                </linearGradient>
                <linearGradient id="recoveryYellow" x1="0" y1="0" x2="0" y2="1">
                ...
                <linearGradient id="recoveryBlue" x1="0" y1="0" x2="0" y2="1">

...
import { Link as LinkIcon, Loader2, Search, Share2 } from 'lucide-react';
...
export function ShareStrategyModal({
  ...
  const buildShareUrl = (share: StrategyShareRow): string => {
    ...
    <div className="space-y-2 pt-4 border-t border-slate-200/60 dark:border-slate-700/50">
      ...
      <div className="space-y-2">
        {existingShares.map((share) => (
          <ShareLinkRow
            key={share.id}
            primaryLabel={`${share.start_date} ~ ${share.end_date}`}
            secondaryLabel={`Created ${new Date(share.created_at).toLocaleDateString()}`}
            shareUrl={buildShareUrl(share)}
            active={share.active}
